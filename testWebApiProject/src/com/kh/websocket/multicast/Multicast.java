   ...
   // 해당 방의 사용자 리스트에 갱신을 해주어야 한다.
    ...
    public void onOpen(EndpointConfig config, Session session, @PathParam("chatRoom") String room) {
       // EndpointConfig : 기본 설정을 적용하겠다.
    	// Session : 현재 접속자의 소캣 정보를 담은 연결 객체(not HttpSession)
    	// @PathParam : 동적인 url 경로를 통해 접근할 경우 해당 경로 부분을 변수로 활용할 수 있게 매개변수로
    	// 변경하여 받을 수 있는 어노테이션
    ...
    public void onMessage(String message, Session session) throws IOException {
    	...
    	// 즉, 나중에 사용자 리스트를 표현하고나 
    	// 어떤 사용자가 보낸 것인지 표현할때 사용할 수 있으므로 코드를 작성 해 놓은 부분
       String chat_id = (String) session.getUserProperties().get("chat_id");
       
       // 현재 데이터를 보낸 사용자의 방 정보를 받아서 같은 방에 있는 사용자들에게 해당 정보를
       // 전달하기 위한 객체
